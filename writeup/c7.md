...
What this means in practice if that each new scope (read: new block), requires its own `variable_map`. Previous the `variable_map` was defined at program scope in `resolve_prgm`:
...
The `variable_map` is discarded by the end of this function because, by the time the function is done, all the user-supplied variable names have been given unique names. The scopes are identfied and done at this point.
...
The data for whether this is shadowing a parent variable (legal) or repeating a local declaration (illegal) is not tracked yet. The simplest way to use a `Entry` type that has both the new name, and a little boolean toggle whether it is local. The dta oriented way is perhaps to create a new map for each new scope, but this seems complicated when the scopes get to deep and you need to keep a stack of maps, etc. I will stick with a boolean, or at least an enum.
...
All that is left is creating a new, duplicate, `variable_map` when resolving a compound statement. Before I do that, I will do some refactorings moving the old `variable_map` into `FuncDef` scope instead, and moving the old `resolve_func_def` logic into a new `resolve_block` that can be reused. This also simplifes later chapters when each function has its own scope as well.
...
Now back to compound statements. First the `variable_map` is copied into a new one, with all the `.local`s changes into `.parent`s. Zig helpfully has a `clone` method. the iterate over all the values and update the `scope`field.
...
This compiles fine. The eye test passes on all C files in chapter 7s tests in the test suite. Time to actually run the test suite, and thankfully all tests pass.
