...
And here is an example of it being used in the tiny `stmt_emit_ir`, which saved on so much paramter space!
...
All `codegen` tests. Pass, which here ismply means it is failing when it should and is not hitting any panics in the passed codepaths. Always a plus.
